# `Implementando a inserção de arquivos/pastas (Fazer Upload)`
...
## `Adicionando o botão de upload (Fazer Upload)`

Vamos começar implementando o botão de `Fazer Upload`:
...
## `Criando o dropdown (com JavaScript) de inserção de arquivos e pastas`
...
## `Criando o formulário (HTML) para capturar arquivos ou pastas do computador`
...
<div id="add-upload-urls"></div>

## `Criando as ROTAS/URLs upload-file/ e upload-folder/`
...
<div id="add-file-upload-validation"></div>

## `Criando um validador de tamanho e tipos de arquivos: validators.py`
...
<div id="add-validate-uploaded-file-utils"></div>

## `Criando a função utilitária _validate_uploaded_file()`

Aqui, nós vamos implementar a função utilitária `_validate_uploaded_file()` que atuará como uma camada intermediária de validação entre o sistema de upload e os validadores definidos em `validators.py`.
...
Se você prestar atenção vai ver que nós estamos chamando outra função chamada `_extract_error_message()` que vai ser responsável por extrair a mensagem de erro de uma exceção.

Vamos implementar ela agora:
...
<div id="valid-exists-name"></div>

## `Validando quando um usuário enviar um arquivo com o nome já existente: _generate_unique_filename()`
...
<div id="create-file-instance"></div>

## `Criando o arquivo no banco de dados: _create_file_instance()`
...
<div id="orchestrating-uploads"></div>

## `Orquestrando o upload de arquivos: _process_single_file_upload()`

Continuando, vamos implementar a função `_process_single_file_upload()` que vai ser responsável por orquestrar todo o fluxo de upload de uma única função, do início ao fim, de forma controlada e previsível.
...
<div id="upload-folder-view"></div>

## `Implementando a view: upload_file()`
...
## `Linkando a view (ação) upload_file() com o formulário de upload`
...
## `Implementando as mensagens de erro no template: upload.html`
...
## `Implementando a view (ação) de inserção de pastas`
