...
Agora, vamos explicar algumas partes do código acima (só o necessário, sem repetir o que já foi explicado em outras partes do README):
 ...
 - `on:`
   ...
   - `push:`
     ...
     - `branches: [ develop ]`
       - Branches que executarão as tarefas, no nosso caso, é *"develop"*;
       - develop já garante a qualidade do código;
...
opendb = 'docker exec -it postgresql_dev psql -U easyrag -d easy_rag_db'
openweb = 'docker exec -it django bash'
opennginx = 'docker exec -it nginx_reverse_proxy bash'
openredis = 'docker exec -it redis_cache bash'
...
restartc = 'docker restart $(docker ps -q)'
rebuildc = 'docker compose up --build -d'
cleanc = """
